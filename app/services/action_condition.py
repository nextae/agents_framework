...
class ActionConditionService:
    ...
    async def try_get_root_for_action_id(
        ...
        operators = await db.exec(
            select(ActionConditionOperator).where(ActionConditionOperator.action_id == action_id)
        )
    ...
    async def get_all_conditions_by_parent_id(
        ...
        operators = await db.exec(
            select(ActionConditionOperator).where(ActionConditionOperator.parent_id == parent_id)
        )
        operators = list(operators.all())
    ...
    async def create_condition_operator(
        ...
        condition_operator = ActionConditionOperator.model_validate(condition_operator_request)

        await ActionConditionService._validate_parent_and_root(
    ...
    async def create_condition_operator_root(
        ...
        root = await ActionConditionService.try_get_root_for_action_id(operator.action_id, db)
        if root:
            raise ConflictError(
                f"Action with id {operator.action_id} already has root assigned with id {root.id}"
            )
    ...
    async def create_condition(
        ...
        if validate:
            try:
                await condition.validate_condition(db)
    ...
    @staticmethod
    async def get_condition_by_id(condition_id: int, db: AsyncSession) -> ActionCondition | None:
    ...
    async def assign_all_operators_by_root_to_action(
        ...
        operators = await ActionConditionService.get_all_conditions_by_root_id(root_id, db)
    ...
    async def update_condition_operator(
        ...
        if operator_update.action_id is not None:
    ...
    async def delete_condition_operator(
        ...
        if cascade:
            children = await ActionConditionService.get_all_conditions_by_parent_id(operator.id, db)
            for child in children:
                if isinstance(child, ActionConditionOperator):
                    await ActionConditionService.delete_condition_operator(child.id, db, True)
    ...
    @staticmethod
    async def _validate_parent_and_root(
